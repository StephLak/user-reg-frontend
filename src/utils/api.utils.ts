...
import { getItemFromStorage, USER_TOKEN_KEY } from "./utils";
...
export const axiosRequestError = (err: any) => {
  const { errors, data, message } = err?.response?.data || {};

  const formattedErrors = handleNetworkErrors({
    error: err,
    validationErrors: errors || data,
  ...
  if (formattedErrors)
    toast({
      status: "error",
      title: "Error",
      description: formattedErrors && formattedErrors[0],
    });
};
